...
def bacteria_extraction(metagenome_k_mers, database_k_mers, k, prefix, dataset, classifier = "multiSVM", batch_size = 32, verbose = 1, saving = 1, cv = 1):
    ...
    if os.path.isfile(bacteria_kmers_file):
        ...
        if classifier in ["oneSVM","lof"]:
            bacteria = bacteria_extraction_oneClass(metagenome_k_mers, database_k_mers, k, prefix, dataset, classifier, batch_size, verbose, saving, cv)
        else:
            bacteria = bacteria_extraction_host(metagenome_k_mers, database_k_mers, k, prefix, dataset, classifier, batch_size, verbose, saving, cv)

    return bacteria

def bacteria_extraction_oneClass(metagenome_k_mers, database_k_mers, k, prefix, dataset, classifier = "oneSVM", batch_size = 32, verbose = 1, saving = 1, cv = 1):
    bacteria_kmers_file = prefix + "_K{}_{}_Xy_bacteria_database_{}_data.h5f".format(k, classifier, dataset)
    ...
    bacteria = extract_bacteria_oneClass(clf, pd.DataFrame(metagenome_k_mers["X"], columns = metagenome_k_mers["kmers_list"], index = metagenome_k_mers["ids"]), bacteria_kmers_file, unclassified_kmers_file, verbose = verbose, saving = saving)

    return bacteria
    """
def bacteria_extraction_host(metagenome_k_mers, database_k_mers, k, prefix, dataset, classifier = "multiSVM", verbose = 1, saving = 1, cv = 1):
    bacteria_kmers_file = prefix + "_K{}_{}_Xy_bacteria_database_{}_data.h5f".format(k, classifier, dataset)
    ...
    bacteria = extract_bacteria_host(clf, pd.DataFrame(metagenome_k_mers["X"], columns = metagenome_k_mers["kmers_list"], index = metagenome_k_mers["ids"]), bacteria_kmers_file, virus_kmers_file, animals_kmers_file, plants_kmers_file, fungi_kmers_file, protists_kmers_file, verbose = verbose, saving = saving)

    return bacteria

def extract_bacteria_host(clf, k_mers, bacteria_kmers_file, virus_kmers_file, animals_kmers_file, plants_kmers_file, fungi_kmers_file, protists_kmers_file, verbose = 1, saving = 1):
    if verbose:
...
def extract_bacteria_oneClass(clf, k_mers, bacteria_kmers_file, unclassified_kmers_file, verbose = 1, saving = 1):
    if verbose:
...
def training(X_train, y_train, kmers, classifier = "OneSVM", batch_size = 32, verbose = 1, cv = 1):
    if classifier == "OneSVM":
        ...
        if verbose:
            print("Training bacterial extractor with VirNet method")
        clf = virnet
    elif classifier == "GeNet":
        if verbose:
            print("Training bacterial extractor with GeNet method")
        clf = genet
    elif classifier == "GRaDL":
        if verbose:
            print("Training bacterial extractor with GRaDL method")
        clf = gradl
        ...
        if verbose:
            ...
            # CHANGE ERROR MESSAGE FOR ACTUAL CLASSIFIERS
    ...
    scaleX(X_train, y_train, batch_size, kmers, nb_classes = 2 if classifier != "oneSVM" else 1)

# TESTER GENERATORS
# Voir si peut utiliser celui de Keras avec sklearn classifiers
    if cv:
        y_pred_test, clf = fit_predict_cv(X_train, y_train, batch_size, kmers, classifier, clf, nb_classes = 2 if classifier != "OneSVM" else 1, cv = cv, shuffle = True)
        training_cross_validation(y_pred_test, list(y_test[0]), classifier)

    else:
        clf = fit_model(X_train, y_train, batch_size, kmers, classifier, clf, nb_classes = 2 if classifier != "OneSVM" else 1, cv = cv, shuffle = True)

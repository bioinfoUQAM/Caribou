...
def construct_data_CPU(Xy_file, dir_path, list_id_file, kmers_list):
    ...
    df = vaex.from_pandas(pd.DataFrame({'kmers':kmers_list}, index = np.arange(len(kmers_list))))
    ids = []
    ...
    for id, file in list_id_file:
        ...
        tmp = vaex.from_csv(file, header = None, names = ['kmers', id])
        # Left join each file to df
        df.join(tmp, on = 'kmers', how = 'left', inplace = True)
    ...
    df.fillna(0, column_names = ids, inplace = True)

    print("NAs filled")

    # Convert to pandas to transpose
    df = df.to_dask_array().transpose()
    print("Converted to pandas")
    # Reconvert to vaex
    df = vaex.from_pandas(df, copy_index = True)
    print("Reconverted to vaex")
    ...
    print("Empty kmers columns dropped")
    ...
    df = None
        ...
        for id, file in list_id_file:
            if df is None:
                try:
                    df = dd.read_csv(file, sep = "\t", header = None, names = ['kmers', id])
                    df = df.set_index('kmers')
                ...
                try:
                    # Read each file individually
                    tmp = dd.read_csv(file, sep = "\t", header = None, names = ['kmers', id])
        ...
        wait(df)
        return save_kmers_profile_CPU(df, Xy_file)

def save_kmers_profile_CPU(df, Xy_file):
    # Extract ids and k-mers from df dataframe + remove kmers column
    kmers_list = np.array(df.index.compute())
    ids = np.array(df.columns.compute())

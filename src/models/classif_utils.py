...
class ClassificationMethods():
    ...
    def __init__(
        ...
        if isinstance(self.database_data, tuple):
            self.host = True
    ...
    def execute_train(self):
        for taxa in self.taxas:
            ...
            if train:
                self.train_model(taxa)

    # Train model according to passed taxa
    def train_model(self, taxa):
    ...
    def _binary_training(self, taxa):
        ...
        if self.classifier_binary == 'onesvm':
            ...
            self._merge_database_host(self.database_data[0], self.database_data[1])
    ...
    def _verify_classifier_binary(self):
        if self.classifier_binary == 'onesvm' and self.host == True:
            ...
            raise ValueError('Classifier {} cannot be used without host data!\nEither add host data to config file or choose the One-Class SVM classifier'.format(self.classifier))
        else:

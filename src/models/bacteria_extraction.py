...
def bacteria_extraction(metagenome_k_mers, database_k_mers, k, outdirs, dataset, training_epochs, classifier = 'deeplstm', batch_size = 32, verbose = True, cv = True):
    ...
    if os.path.isfile(bacteria_data_file):
        ...
        if classifier == 'onesvm' and isinstance(database_k_mers, tuple):
            ...
            print('type of list : ', type(list(X_train.to_modin()['id'])[0]))
            print("type of y_train['id'] : ", type(y_train['id'][0]))
            print('mask')
            mask = y_train['id'].isin(list(X_train.to_modin()['id']))
            print(mask)
            y_train = ray.data.from_modin(y_train[mask])

...
class ModelsUtils(ABC):
    ...
    def _cv_score(self, y_true, y_pred):
        ...
        if self.classifier in ['onesvm','linearsvm', 'attention','lstm','deeplstm']:
            support = precision_recall_fscore_support(y_true.to_pandas(), y_pred['classes'], pos_label = 'bacteria', average = 'binary')
        elif self.classifier in ['sgd','svm','mlr','mnb','lstm_attention','cnn','widecnn']:
            support = precision_recall_fscore_support(y_true.to_pandas(), y_pred['classes'], average = 'macro')
    ...
    def _fit_model(self, df):
        ...
        with parallel_backend('ray'):
            if self.classifier == 'onesvm':
                ...
                self._clf.fit(X.to_pandas(), y.to_pandas())
                # for epoch in np.arange(self._training_epochs):
